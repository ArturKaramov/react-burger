...
type Props = {
  ...
  buttonClick: (value: TInputValue) => void;
};

export const Form: FC<Props> = ({ title, inputs, button, links, buttonClick }) => {
  const getInitValues = () => {
    const initValue: TInputValue = {}
    inputs.map((input) => (initValue[input.name] = ""));
    return initValue
  }
  const initValue: TInputValue = getInitValues();
  
  const [value, setValue] = React.useState<TInputValue>(initValue);

  const isDisabled = useMemo(
    () => Object.values(value).some((item) => {return item.length === 0}),
    [value]
  );
